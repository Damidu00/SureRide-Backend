// import Booking from '../models/booking.js'
// import cars from '../models/carsModel.js'
// import users from '../models/user.js'


// export async function createBooking(req, res) {
//   try {
//     const { car, startDate, endDate, totalPrice } = req.body;
//     const user = req.user.id;

//     if (!car || !startDate || !endDate || !totalPrice) {
//       return res.json({
//         message: "All fields are required 😕"
//       });
//     }

//     const booking = new Booking({
//       user,
//       car,
//       startDate,
//       endDate,
//       totalPrice,
//     });

//     await booking.save();

//     res.json({
//       message: "Booking created successfully ✅",
//       booking,
//     });
//   } catch (error) {
//     res.json({
//       message: "Booking not created ❌",
//       error: error.message,
//     });
//   }
// }

// export async function getAllBookings(req, res) {
//   try {
//     const bookings = await Booking.find()
//       .populate('user', 'firstName lastName email')
//       .populate('car', 'brand model registrationNumber');
//     res.status(200).json({
//       success: true,
//       bookings: bookings
//     });
//   } catch (error) {
//     res.status(500).json({
//       success: false,
//       message: "Error fetching bookings",
//       error: error.message
//     });
//   }
// }

// export async function getbookingById(req, res) {
//   try {
//     const bookingId = req.params.id;
//     const booking = await Booking.findById(bookingId)
//       .populate('user', 'firstName lastName email')
//       .populate('car', 'brand model registrationNumber');
//     if (!booking) {
//       return res.status(404).json({
//         success: false,
//         message: 'Booking not found'
//       });
//     }
//     res.status(200).json({
//       success: true,
//       booking: booking
//     });
//   } catch (error) {
//     res.status(500).json({
//       success: false,
//       message: 'Error fetching booking',
//       error: error.message
//     });
//   }
// }





import Booking from '../models/booking.js'
import Car from '../models/carsModel.js'
import users from '../models/user.js'


function normalizeDate(d) {
  const dt = new Date(d);
  dt.setHours(0, 0, 0, 0);
  return dt;
}

function daysBetweenInclusive(start, end) {
  const msPerDay = 24 * 60 * 60 * 1000;
  const s = normalizeDate(start);
  const e = normalizeDate(end);
  const diff = Math.round((e - s) / msPerDay);
  return diff + 1; // inclusive of both start and end
}

/**
 * Check if two date ranges overlap (inclusive)
 */
function rangesOverlap(aStart, aEnd, bStart, bEnd) {
  return aStart <= bEnd && bStart <= aEnd;
}

export async function createBooking(req, res) {
  try {
    const { car, startDate, endDate, totalPrice } = req.body;
    const user = req.user?.id;

    // require authentication
    if (!user) {
      return res.status(401).json({ message: 'Authentication required' });
    }

    if (!car || !startDate || !endDate) {
      return res.status(400).json({ message: "car, startDate and endDate are required" });
    }

    const sDate = normalizeDate(startDate);
    const eDate = normalizeDate(endDate);

    // validate dates using getTime() to ensure valid Date objects
    if (isNaN(sDate.getTime()) || isNaN(eDate.getTime()) || sDate > eDate) {
      return res.status(400).json({ message: "Invalid date range" });
    }

    // ensure car exists
    const carDoc = await Car.findById(car);
    if (!carDoc) {
      return res.status(404).json({ message: "Car not found" });
    }

    // check overlapping bookings for the same car (ignore cancelled)
    const overlapping = await Booking.findOne({
      car: car,
      status: { $ne: 'cancelled' },
      $or: [
        { startDate: { $lte: eDate }, endDate: { $gte: sDate } },
      ],
    });

    if (overlapping) {
      return res.status(409).json({ message: "Car is not available for the selected period" });
    }

    const days = daysBetweenInclusive(sDate, eDate);
    const calculatedTotal = days * (carDoc.pricePerDay || 0);

    const booking = new Booking({
      user,
      car,
      startDate: sDate,
      endDate: eDate,
      totalPrice: typeof totalPrice === 'number' ? totalPrice : calculatedTotal,
    });

    await booking.save();

    res.status(201).json({
      message: "Booking created successfully",
      booking,
      meta: { days, calculatedTotal }
    });
  } catch (error) {
    res.status(500).json({
      message: "Booking not created",
      error: error.message,
    });
  }
}

export async function getAllBookings(req, res) {
  try {
    const bookings = await Booking.find()
      .populate('user', 'firstName lastName email')
      .populate('car', 'brand model registrationNumber');
    res.status(200).json({
      success: true,
      bookings: bookings
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Error fetching bookings",
      error: error.message
    });
  }
}

export async function getbookingById(req, res) {
  try {
    const bookingId = req.params.id;
    const booking = await Booking.findById(bookingId)
      .populate('user', 'firstName lastName email')
      .populate('car', 'brand model registrationNumber');
    if (!booking) {
      return res.status(404).json({
        success: false,
        message: 'Booking not found'
      });
    }
    
    res.status(200).json({
      success: true,
      booking: booking
    });
  } catch (error) {
    res.status(500).json({
      success: false,
      message: 'Error fetching booking',
      error: error.message
    });
  }
}

export async function getAvailableCars(req, res) {
  try {
    const { startDate, endDate } = req.query;
    if (!startDate || !endDate) {
      return res.status(400).json({ message: 'startDate and endDate query params are required' });
    }

    const sDate = normalizeDate(startDate);
    const eDate = normalizeDate(endDate);
    if (isNaN(sDate) || isNaN(eDate) || sDate > eDate) {
      return res.status(400).json({ message: 'Invalid date range' });
    }

    // find cars that have no overlapping non-cancelled bookings
    const busyBookings = await Booking.find({
      status: { $ne: 'cancelled' },
      startDate: { $lte: eDate },
      endDate: { $gte: sDate }
    }).select('car');

    const busyCarIds = busyBookings.map(b => String(b.car));

    const available = await Car.find({ _id: { $nin: busyCarIds }, availability: true });

    res.status(200).json({ success: true, available });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Error checking availability', error: error.message });
  }
}

